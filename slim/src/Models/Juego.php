...
class Juego extends Modelo
{
  ...
  public function obtenerJuego($id)
  {
    try {
      ...
      if ($juego) {
        ...
        $tablaCalificaciones = $connection->prepare('SELECT * FROM calificacion WHERE juego_id = :juego_id');
  ...
  public function obtenerJuegos($pagina, $clasificacion, $texto, $plataforma)
  {
    ...
    try {
      ...
      $consulta = "SELECT j.*, AVG(c.estrellas) AS puntuacion_promedio FROM juego j
      ...
      if(!empty($clasificacion)){
        ...
        }elseif($clasificacion === '13' || $clasificacion === ' 13'){
          $consulta .= ' AND j.clasificacion_edad = :clasificacion';
          $parametros['clasificacion'] = '+13';
        }elseif($clasificacion === '18' || $clasificacion === ' 18'){
          $consulta .= ' AND j.clasificacion_edad = :clasificacion';
      ...
      $consulta .= ' GROUP BY j.id ORDER BY puntuacion_promedio DESC LIMIT :limite OFFSET :offset';
      
      $tablaJuego = $connection->prepare($consulta);
      //Asigno los parametros
      if(isset($parametros['texto'])){
        $tablaJuego->bindParam(':texto', $parametros['texto'], PDO::PARAM_STR);
      }
      if(isset($parametros['clasificacion'])){
        $tablaJuego->bindParam(':clasificacion', $parametros['clasificacion'], PDO::PARAM_STR);
      }
      if(isset($parametros['plataforma'])){
        $tablaJuego->bindParam(':plataforma', $parametros['plataforma'], PDO::PARAM_STR);
      }
      $tablaJuego->bindParam(':limite', $limite, PDO::PARAM_INT);
      $tablaJuego->bindParam(':offset', $offset, PDO::PARAM_INT);

      $tablaJuego->execute();
      
      $juegos = $tablaJuego->fetchAll(PDO::FETCH_ASSOC);
      if($juegos){
      $this->data['Status'] = 'Success';
      $this->data['Codigo'] = 200;
      $this->data['Data'] = $juegos;
     // $this->data['Data']['Consulta'] = $consulta;
    }else{
      $this->data['Status'] = 'Fail';
